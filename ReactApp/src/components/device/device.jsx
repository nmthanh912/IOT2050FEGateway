...
export default function EdgeDevice({ data, onDetail, isRunning }) {
	...
	const exportToCSV = () => {
		...
		async function loadData() {
			try {
				...
				const res = await DeviceService.getConfigInfoById(
					data.ID,
					data.protocol
				);
	...
	const deleteDevice = () => {
		...
		confirm &&
			DeviceService.deleteDevice(data.ID)
				.then((res) => {
	...
	const turnOnDevice = () => {
		HardwareServices.poweron(data.protocol, data.ID)
			.then(res => {
	...
	const shutdownDevice = () => {
		HardwareServices.shutdown(data.protocol, data.ID).then(res =>
			console.log(res.data)
		);
	};
			...
			>
				<DropdownItem.Header>
					<div className="row justify-content-between">
						<div className="text-primary col-3">
							<u>#{data.ID}</u>
						</div>
